...
- [Resources](#resources)
- [Information Gathering Flowchart](#information-gathering-flowchart)
...
## Information Gathering Flowchart
...
flowchart LR
    ...
    subgraph Passive
        B[DNS / WHOIS]
        C[Subdomains]
        D[Certificates / CT Logs]
    end
    subgraph Active
        E[Port Scan]
        F[Banner Grab / Services]
        G[Version / Script Scan]
    end
    subgraph Enumerate
        H[SMTP / SNMP / etc.]
        I[Document & Report]
...
| Subject | Description |
| --- | --- |
| Additional resources | See Resources table and linked documentation for more depth. |
| Recon phases | Passive → Active → Enumerate; align with flowchart above. |

## More tables

| Reference | Location |
| --- | --- |
| Common ports | See Common Ports and Domain Controller ports tables in this handbook. |
| TTL / TCP window | See Time To Live (TTL) and TCP Window Size Values table. |

## Tools and commands

| Category | Example |
| --- | --- |
| DNS | `dig`, `host`, `nslookup` — see DNS section. |
| Port scan | `nmap`, `naabu`, `masscan` — see Nmap and Port Scanning sections. |
| Subdomains | `amass`, `subfinder`, `dnsenum` — see respective sections. |

## Payloads table

| Type | Description | Reference |
| --- | --- | --- |
| Recon / discovery | Queries, scan options, wordlists | See DNS, Nmap, subfinder, Amass sections above. |
| Banner / service | Connection strings, probes | See Banner Grabbing, Port Scanning sections. |
