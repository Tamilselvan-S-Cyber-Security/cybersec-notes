...
- [Resources](#resources)
- [Security Architecture Flowchart](#security-architecture-flowchart)
...
flowchart LR
    subgraph Design
        A[Requirements / Threat Model]
    end
    subgraph Controls
        B[Hardening / Segmentation]
        C[Least Privilege / Monitoring]
    end
    subgraph Validate
        D[Test / Review]
    end
...
| --- | --- | --- |
| Hardening | LAPS, Device Guard, Mitigate Kerberoast | See LAPS, Device Guard, Mitigate sections above. |
| Red Forest / PAW | Architecture patterns | See Red Forest, Privileged Administrative Workstations. |
