...
| IR phases | Detect → Triage → Investigate → Contain → Recover. |

## More tables

| Reference | Location |
| --- | --- |
| Wireshark filters | See Wireshark Filters and Logical Operators in this handbook. |
| Tool references | See Resources table for FIR, TheHive, Loki, etc. |

## Tools and commands

| Category | Example |
| --- | --- |
| Capture | `tcpdump`, Wireshark — see Wireshark section. |
| Filters | `ip.addr == <RHOST>`, `tcp.port == <RPORT>` — see Filters table. |

## Payloads table

| Type | Description | Reference |
| --- | --- | --- |
| IOC / hunt | Queries, filters, artifact hashes | See Wireshark Filters, Resources (Loki, Fenrir). |
| IR playbooks | Collection, triage, containment | See Resources (FIR, TheHive, IRIS). |
