...
- [Resources](#resources)
- [OSINT Flowchart](#osint-flowchart)
...
flowchart LR
    subgraph Collect
        A[Target / Query]
        B[Search Engines / Dorks]
    end
    subgraph Gather
        C[Emails / Domains / Social]
        D[Breach / Certificate Data]
    end
    subgraph Analyze
        E[Correlate / Map]
    end
...
| Subject | Description |
| --- | --- |
| Additional resources | See Resources (Recon-ng, Sherlock, GHunt, etc.). |
| OSINT flow | Collect → Gather → Analyze → Report; see flowchart. |

## More tables

| Reference | Location |
| --- | --- |
| Tools | theHarvester, Photon, Recon-ng, Social Analyzer in sections above. |
| Dorks / search | Fast Google Dorks Scan, Google section. |

## Tools and commands

| Category | Example |
| --- | --- |
| theHarvester | `theHarvester -d <DOMAIN> -l 500 -b google -f myresults.html` |
| Recon-ng | See Social Analyzer section for CLI commands. |

## Payloads table

| Type | Description | Reference |
| --- | --- | --- |
| Search / dorks | Google dorks, search queries | See Fast Google Dorks Scan, Google section. |
| Username / email | Enumeration lists, formats | See theHarvester, Sherlock, h8mail in sections. |
